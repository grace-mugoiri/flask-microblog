...
from app.main.forms import EditProfileForm, EmptyForm, PostForm, SearchForm, \
	MessageForm
from app.models import User, Post, Message, Nototification
...
def index():
    ...
    posts = current_user.followed_posts().paginate(
        page, current_user.config['POST_PER_PAGE'], False)
    next_url = url_for(
...
def explore():
    ...
    posts = Post.query.order_by(Post.timestamp.desc()).paginate(
        page, current_app.config['POST_PER_PAG'], False)
    next_url = url_for(
...
def edit_profile():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash(_('Your changes have been saves'))
